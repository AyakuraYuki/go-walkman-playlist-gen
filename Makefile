...
release: check-env-release
	...
	CGO_ENABLED=0 GOOS=$(GOOS) GOARCH=$(GOARCH) go build -mod=vendor -ldflags "-s -w -X main.buildVersion=$(VERSION)" -o $(BUILD_DIR)/$(NAME)$(ext) main.go
